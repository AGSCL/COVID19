...
Good country index (v. 1.3) measures how much each of the countries contribute to the planet and the human race through their policies and behaviors, consistent in Science & Technology, Culture, International Peace & Security, World Order, Planet & Climate, Prosperity & Equality, and Health & Well-being [link](https://www.goodcountry.org/index/source-data/). Covers153 countries. For more information about this index, please go to the following [link](https://www.goodcountry.org/index/source-data/).
...
coronavirus_iso3c_9<- coronavirus_iso3c_8%>%
        dplyr::left_join(dplyr::select(wb_SP.POP.TOTL,iso3c,date,SP.POP.TOTL),
                         by=c("iso_code"="iso3c"))%>%
        dplyr::rename("total_pop"="SP.POP.TOTL", "year_total_pop"="date")%>%
        dplyr::left_join(dplyr::select(wb_EN.POP.DNST,iso3c,date,EN.POP.DNST),
                         by=c("iso_code"="iso3c"))%>%  
        dplyr::rename("pop_density"="EN.POP.DNST", "year_pop_density"="date")%>%
        dplyr::left_join(dplyr::select(wb_SH.MED.BEDS.ZS,iso3c,date,SH.MED.BEDS.ZS),
                         by=c("iso_code"="iso3c"))%>%
        dplyr::rename("hosp_beds_per_1000"="SH.MED.BEDS.ZS", "year_hosp_beds_per_1000"="date")%>%
        dplyr::left_join(dplyr::select(wb_EN.ATM.PM25.MC.M3,iso3c,date,EN.ATM.PM25.MC.M3),
                         by=c("iso_code"="iso3c"))%>%
        dplyr::rename("air_pollution_year_pm25"="EN.ATM.PM25.MC.M3", "year_air_pollution_year_pm25"="date")%>%
        dplyr::left_join(dplyr::select(wb_SP.DYN.LE00.IN,iso3c,date,SP.DYN.LE00.IN),
                             by=c("iso_code"="iso3c"))%>%
        dplyr::rename("life_exp"="SP.DYN.LE00.IN", "year_life_exp"="date") %>% 
        dplyr::left_join(dplyr::select(wb_SP.DYN.CDRT.IN,iso3c,date,SP.DYN.CDRT.IN),
                             by=c("iso_code"="iso3c"))%>%
        dplyr::rename("death_rate_crude"="SP.DYN.CDRT.IN", "year_death_rate_crude"="date")
```
...
```{r who_mortality1, echo=T, cache= T, paged.print=TRUE, eval=T, error=T}
...
```{r linear_deaths_who_plot, echo=T, cache= T, paged.print=TRUE,eval=T, error=T, fig.cap="Figure 5. Linear Trends of Deaths By Year"}
Sys.setlocale(category = "LC_ALL", locale = "english")
...
col_names_df<-coronavirus_iso3c_13%>%
    dplyr::select(population:TemperatureAvg)%>% 
    names()

coronavirus_iso3c_13%>%
  dplyr::group_by(country) %>% 
    summarise_at(vars(population:TemperatureAvg),~sum(is.na(.))/n())%>%
      dplyr::mutate(sumVar = rowSums(dplyr::select(., .dots = all_of(col_names_df))))%>%
      dplyr::mutate_at(vars(population:TemperatureAvg),~scales::percent(.))%>%
    dplyr::filter(sumVar>0)%>%
    dplyr::arrange(desc(sumVar))%>%
  dplyr::rename("sum_of_miss"="sumVar")%>%
  dplyr::select(country,sum_of_miss, everything())%>%
...
vim_agg<-
  VIM::aggr(coronavirus_iso3c_13, col=c('navyblue','yellow'),
...
month_temp_16, month_rainfall_mm_16,month_temp_agg97_06,month_temp_agg07_16,month_rain_agg97_06,month_rain_agg07_16, TemperatureAvg))%>% 
  tidyr::unnest()%>%
  dplyr::group_by(iso_code)%>%
  dplyr::mutate_at(vars(dates_1:TemperatureAvg_120),~max(.,na.rm=T))%>% #(ncol(.)-1)
